...
from dataset import KEMD20_Text
...
def main():
    ...
    train_dataset = KEMD20_Text(tokenizer, './train_data.tsv', label_file_name='./train_labels.csv')
    ...
    model = CoMPM(args.pretrained, 7, special_token, tokenizer.cls_token_id, tokenizer.pad_token_id, len(tokenizer), attention=args.att)
    model = model.cuda()
    model.train()
